...
typedef struct _LEAKED_LIST
{  // Holds memory leaks so they can be printed nicely.
  unsigned int source_line;
...
int MT_modify_entry(unsigned long int old_address, unsigned long int new_address, unsigned int size, const char *TLA, unsigned int source_line, const char *function_name, const char *file_name);
void MT_dump_stats();
void MT_dump_hash();
...
void *MT_Calloc(size_t numMembers, size_t size, int line, const char *func_name, const char *file_name);
void *MT_Realloc(void *ptr, size_t size, int line, const char *func_name, const char *file_name);
...
int MT_is_deallocater_wrong(const char *alloc_TLA, const char *dealloc_TLA)
{
  ...
  if ((strcmp(dealloc_TLA, "FRE") == 0 || strcmp(dealloc_TLA, "RAL") == 0)
      && (strcmp(alloc_TLA, "MAL") == 0 || strcmp(alloc_TLA, "CAL") == 0
          || strcmp(alloc_TLA, "RAL") == 0)) { return ok; }

  if (strcmp(dealloc_TLA, "DEL") == 0 /* C++ scalar delete */
      && strcmp(alloc_TLA, "NEW") == 0) return ok;

  if (strcmp(dealloc_TLA, "VDE") == 0 /* C++ vector delete */
      && strcmp(alloc_TLA, "VEC") == 0) return ok;
...
void MT_dump_hash()
{
  ...
  for (i = 0; i < _hashtable.total_rows; i++) {
    ...
    printf("\nrow[%u]", i);
    while (mover) {
      printf("n=%u:a=0x%lX:size=%u  ", mover->sequence_num, mover->address, mover->size_mem);
      mover = mover->next;
    }
  }
}
...
void *MT_Calloc(size_t numMembers, size_t size, int line, const char *func_name, const char *file_name)
{
  ...
  memset(ptr, 0, size);
  MT_add_entry((unsigned long int)ptr, size, "CAL", line, func_name, file_name);
  MT_init_redzone(ptr, size);
  return ptr;
}
...
void *MT_Realloc(void *ptr, size_t size, int line, const char *func_name, const char *file_name)
{
  ...
  if (!ptr) {
    ...
    MT_modify_entry((unsigned long int)ptr, (unsigned long int)newptr, size, "RAL", line, func_name, file_name);
  }
